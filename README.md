...
The token is a read-only bearer capability for one artifact version. Your job as
the agent is to open the view for the human, not make them copy-paste a string.

### 7. Open the view for your human and print the fallback URL
...
If you are running with access to the human's desktop, open the browser for them
and also print the URL as a fallback/click-through. On macOS use `open`; on
Linux use `xdg-open`. If you are headless or opening fails, just print the URL.
...
### 8. Check the public response

```bash
curl -fsS "$SERVER_ORIGIN/present/$TOKEN"   | tee evidence/12-public-present-response.json   | jq '{keys: keys, expires_at, op_count: (.a2ui.a2ui_operations | length)}'
```
